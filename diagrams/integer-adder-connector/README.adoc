...
:short-bonita-version: 7.14
:doc-url: https://documentation.bonitasoft.com/bonita/7.14
:java-version: 11
= integer-adder-connector

The project **integer-adder-connector** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/integer-adder-connector.git
cd integer-adder-connector/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `integer-adder-connector-1.0-SNAPSHOT.jar`
